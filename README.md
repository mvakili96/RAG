...
Retrieval sources belong in `documents/` which is git-ignored. Models, prompts, the query, chunking, indexing, and retrieval parameters are defined in `config.yml`. On the first run, use indexing mode `build` to create and persist both Flat and HNSW indexes under `index-data/`. Then switch to `load` to reuse them and select either index with `dense_index_type`. Install dependencies with `pip install -r requirements.txt`, then run `python main.py`.
